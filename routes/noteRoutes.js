...
// GET ALL NOTES
// GET ALL NOTES with Pagination + Sorting
router.get("/", async (req, res) => {
  try {
    ...
    if (res.locals.user) {
      ...
      const totalNotes = await Note.countDocuments({ userId: res.locals.user.id });

      notes = await Note.find({ userId: res.locals.user.id })
